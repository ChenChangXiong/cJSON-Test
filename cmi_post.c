...
//请求报文
char* request_message(void)
{
	cJSON *root, *date, *fld, *arry;
	char i = 0 ;
	struct request_buf request_pack[2] = {
		{"DeviceNO1", {"p1","p2","p3"}, "2019-02-28T16:00:00.000Z", "2019-03-21T16:00:00.000Z"},
		{"DeviceNO2", {"n1","n2","n3"}, "2019-02-28T16:00:00.000Z", "2019-03-21T16:00:00.000Z"},
	} ;
	...
	cJSON_AddItemToObject(root, "header", fld=cJSON_CreateObject());
	cJSON_AddStringToObject(fld, "ContextType", "Application/Json");
	cJSON_AddStringToObject(fld, "X-Api-Version", "1");
	cJSON_AddStringToObject(fld, "X-Request-Id", "20190101123456789");
    //添加数组
	cJSON_AddItemToObject(root, "Queries", fld=cJSON_CreateArray());
	for(i=0;i<2;i++)
	{
		cJSON_AddItemToArray(fld, date=cJSON_CreateObject());
		cJSON_AddStringToObject(date, "DeviceNo", request_pack[i].deviceNo);
		//"Points": ["p1","p2","p3"]	
		cJSON_AddItemToObject(date, "Points", arry=cJSON_CreateArray());
		cJSON_AddStringToObject(arry, "", request_pack[i].points[0]);
		cJSON_AddStringToObject(arry, "", request_pack[i].points[1]);
		cJSON_AddStringToObject(arry,"",  request_pack[i].points[2]);
		//
		cJSON_AddStringToObject(date, "From", request_pack[i].startTime);
		cJSON_AddStringToObject(date, "To", request_pack[i].endTime);
	}
	out=cJSON_Print(root);	cJSON_Delete(root);	free(out);	/* Delete the cJSON, print it, release the string.*/
